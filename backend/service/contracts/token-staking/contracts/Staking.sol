...
contract Staking {
     ...
     */
    function lockTokens(uint72 tokenAmount_, uint24 lockingPeriodInBlocks_, address user) public poolTokenSet {
        ...
        require(poolToken.transferFrom(user, address(this), tokenAmount_), "TransferFrom of poolTokens failed");
     ...
     */
    function unlockTokens(address user) public poolTokenSet {
        Stake memory stake = stakes[user];

        uint256 stakeAmount = stake.tokenAmount;

        require(stakeAmount != 0, "User does not have a stake to unlock");
        ...
        _punishEarlyWithdrawal(stake, user);

        // Zero the Stake - to protect from double-unlocking and to be able to stake again
        delete stakes[user];

        require(poolToken.transfer(user, stakeAmount), "Pool token transfer failed");
    }
    
    /**
     * @notice If the unlock is called prematurely - we subtract the bonus
     */
    function _punishEarlyWithdrawal(Stake memory stake_, address user) internal {
        ...
        if (block.number < (stake_.startBlock + stake_.lockingPeriodInBlocks)) { // lt - cause you can only withdraw at or after startBlock + lockPeriod
            rewardPointsEarned[user] -= stake_.expectedStakingRewardPoints;
            totalRewardPoints -= stake_.expectedStakingRewardPoints;
            emit StakeUnlockedPrematurely(user, stake_.tokenAmount, stake_.lockingPeriodInBlocks, block.number - stake_.startBlock);
        } else {
            emit StakeUnlocked(user, stake_.tokenAmount, stake_.lockingPeriodInBlocks, stake_.expectedStakingRewardPoints);
        }
    }
     ...
     */
    function getRewards(address user) public {
        require(block.number > stakingProgramEndsBlock, "You can only get Rewards after Staking Program ends");
        require(stakes[user].tokenAmount == 0, "You still have a stake locked - please unlock first, don't leave free money here");
        require(rewardPointsEarned[user] > 0, "You don't have any rewardPoints");
        ...
        uint256 amountEarned = stakingFundAmount * rewardPointsEarned[user] / totalRewardPoints;
        rewardPointsEarned[user] = 0; // Zero rewardPoints of a user - so this function can be called only once per user

        _grantTokens(user, amountEarned); // Grant OIL reward earned by user for future vesting during the Vesting period
    }
    ...
    function release(address user) public {
        uint256 releasable = _releasableAmount(user);
        require(releasable > 0, "Vesting release: no tokens are due");

        releasedTokens[user] += releasable;
        require(oilerToken.transfer(user, releasable), "Reward oilers transfer failed");

        emit grantedTokensReleased(user, releasable);
    }
